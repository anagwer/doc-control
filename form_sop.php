...
session_start();
	...
	echo "<script>window.location.href='index.php/sop/add-sop';</script>";
	}
...
if(isset($_POST['submit'])){ //jika tombol submit di set atau dijalankan maka melakukan aksi
	...
	$target_file = $target_dir . basename($_FILES["filesop"]["name"]);
	$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
	if ($_FILES["filesop"]["size"] > 1000000000) {
...
//}
	...
	if (move_uploaded_file($_FILES["filesop"]["tmp_name"], $target_file)) {
			echo "File". basename( $_FILES["filesop"]["name"]). " berhasil diupload.";

		} else {
			echo "Tidak ada file yang diunggah.";
		}
		
		$filename = basename($_FILES['filesop']['name']);
		}
	
	
	//deklarasi variable $table, $field yang akan digunakan untuk fungsi insert data
		$table = "INSERT INTO sop SET";
...
<table align="center">
	...
	<tr>
		<td>Tambah file</td>
		<td>:</td>
		<td><input type="file" name="filesop" class='btn btn-success'></td>
	</tr>
	<tr>
	<tr>
...
<tr>
	<tr>
		<th bgcolor='silver' style='text-align:center;' rowspan='2'>No</th>
		...
		<th bgcolor='silver' style='text-align:center;' colspan='2'>Tanggal Terbit</th>
