...
<?php
	...
	//Perintah Delete
		...
		$qryDelete = "DELETE FROM datasheet WHERE id_sop='$id_sop'";
		...
		echo "<script>window.location.href='index.php/datasheet/add-datasheet';</script>";
		}	
...
<?php
	$tampil = mysql_fetch_array(mysql_query("SELECT * FROM datasheet WHERE id_sop = '".$_GET['id_sop']."'")); 
	{
	?>
	<?php	
		 if(!empty($err))  {
...
<form method="POST" action="" enctype="multipart/form-data" >
	<table align="center">
		...
		<tr>
			...
			<td>:</td>
			<td><input type="text" name="judul_formulir" value="<?=$tampil['judul_formulir']?>" class="form-control" required/></td>
		</tr>
		<tr>
			<td>Departemen Pemilik</td>
			<td>:</td>
			<td><input type="text" name="departement_pemilik" value="<?=$tampil['departement_pemilik']?>" class="form-control" required/></td>
		</tr>
		<tr>
		<!--td>Tambah file</td>
			<td>:</td>
			<td><input type="file" name="filedatasheet" class='btn btn-primary'></td>
		</tr-->
		<?php } ?>
		<tr>
