...
<?php
	...
	if ($_GET['key']=='hapus'){
		...
		$qryDelete = "DELETE FROM standard_doc WHERE id_sop='$id_sop'";
		$del = mysql_query("$qryDelete")or die ('Error!!'.mysql_error());
		if ($del){
			?><script>alert("Data terhapus.");</script>
			<?php
			echo "<script>standard_docndow.location.href='index.php/manual-policy/addmanual-policy';</script>";
	}
	}
	...
	if ($_FILES["filepolice"]["size"] > 500000) {
...
//}
		...
		echo "<script>window.location.href='index.php/standard-doc/edit-form-standard-doc';</script>";
		exit;
	}
	}
	//------------------------------------------------------------------
...
<?php
$tampil = mysql_fetch_array(mysql_query("SELECT * FROM standard_doc WHERE id_sop = '".$_GET['id_sop']."'")); 
{
?>
<?php	
	 if(!empty($err))  {
...
<?php
	...
	echo "<table class='table table-striped table-bordered table-condensed bootstrap-datatable datatable' cellspacing='0' cellpadding='0' standard_docdth='80%' align ='center' border ='1'>
	<tr>
		<tr>
