...
<?php
	...
	if ($_GET['key']=='hapus'){
		...
		$qryDelete = "DELETE FROM report_audit WHERE id_report='$id_report'";
...
<?php
	$tampil = mysql_fetch_array(mysql_query("SELECT * FROM report_audit WHERE id_report = '".$_GET['id_report']."'")); 
	{
	?>
	<?php	
		 if(!empty($err))  {
...
$select = "SELECT * FROM report_audit";
