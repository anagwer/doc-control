...
<?php
	...
	if ($_GET['key']=='hapus'){
		...
		$qryDelete = "DELETE FROM report_lain_lain WHERE id_report='$id_report'";
...
//}
	...
	if (move_uploaded_file($_FILES["file_report"]["tmp_name"], $target_file)) {
		...
		//deklarasi variable $table, $field yang akan digunakan untuk fungsi insert data
			$table = "UPDATE report_lain_lain SET";
...
<?php
	$tampil = mysql_fetch_array(mysql_query("SELECT * FROM report_lain_lain WHERE id_report = '".$_GET['id_report']."'")); 
	{
	?>
	<?php	
		 if(!empty($err))  {
