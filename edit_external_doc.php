...
// untuk memulai session pada fungsi login
	...
	if ($_GET['key']=='hapus'){
		$code_document = $_GET['id_sop'];
		
		$qryDelete = "DELETE FROM external_doc WHERE code_document='$code_document'";
		$del = mysql_query("$qryDelete")or die ('Error!!'.mysql_error());
		if ($del){
			?><script>alert("Data terhapus.");</script>
			<?php
			echo "<script>window.location.href='index.php/external-doc/edit-form-external-doc';</script>";
	}
	}
	...
	if ($_FILES["fileextdoc"]["size"] > 100000000) {
	...
	if (move_uploaded_file($_FILES["fileextdoc"]["tmp_name"], $target_file)) {
		...
		//deklarasi variable $table, $field yang akan digunakan untuk fungsi insert data
			...
			$where = "WHERE id_sop='".$_GET['id_sop']."'";
...
<?php
	$tampil = mysql_fetch_array(mysql_query("SELECT * FROM external_doc WHERE id_sop = '".$_GET['id_sop']."'")); 
	{
	?>
	<?php	
		 if(!empty($err))  {
...
<table align="center">
	...
	<tr>
		<td>Penerbit</td>
		<td>:</td>
		<td><input type="text" name="penerbit" value="<?=$tampil['penerbit']?>" class="form-control" required/></td>
	</tr>
	<tr>
	...
	<!--tr>
		<td>Tambah file</td>
		<td>:</td>
		<td><input type="file" name="fileextdoc" class='btn btn-success'></td>
	</tr-->
	<?php } ?>
	<tr>
...
<tr>
	<tr>
		...
		<th bgcolor='silver' style='width:130px; padding-top:15px;text-align:center;'>Action</th>
	</tr>
	</tr>
</tr>";
$no=0;
...
echo "<tr>
	...
	echo "<td><a href='uploads_file/doc_external_doc/".$file_ext_doc."' title=".$file_ext_doc.">".$code_document."</a></td>";
	}else{ 
	...
	<td>
		...
		<a href='?key=hapus&id_sop=$code_document' class='btn btn-danger btn-xs'>Hapus</a>
	</td>
	</tr>";
}
echo"</table>";
