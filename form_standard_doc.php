...
if(isset($_POST['submit'])){ //jika tombol submit di set atau dijalankan maka melakukan aksi
	...
	if ($_FILES['filestandardoc']['name']!=''){
	$target_dir = "uploads_file/doc_standar_doc/";
$target_file = $target_dir . basename($_FILES["filestandardoc"]["name"]);
$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
if ($_FILES["filestandardoc"]["size"] > 500000) {
    echo "Maaf, file terlalu besar. Maksimal 500KB.";
	//upload digagalkan
	echo "<script>window.location.href='index.php/standard-doc/add-standard-doc';</script>";
}

if (move_uploaded_file($_FILES["filestandardoc"]["tmp_name"], $target_file)) {
        echo "File". basename( $_FILES["filestandardoc"]["name"]). " berhasil diunggah.";
	...
	$filename = basename($_FILES['filestandardoc']['name']);
...
<table align="center">
	...
	<td>Tambah file</td>
	<td>:</td>
	<td><input type="file" name="filestandardoc" class='btn btn-primary'></td></tr>
	<tr>
	</tr>
	<tr>
...
echo "<tr>
	...
	//echo "<td><a href='http://DC/DC/uploads_file/doc_standar_doc/".$file_stand_doc."' title=".$file_stand_doc.">".$code_document."</a></td>";
