...
<?php
	...
	if ($_GET['key']=='hapus'){
		...
		$qryDelete = "DELETE FROM archieve WHERE id_arc='$id'";
	$del = mysql_query("$qryDelete")or die ('Error!!'.mysql_error());
	if ($del){
		?><script>alert("Data terhapus.");</script>
		<?php
		echo "<script>window.location.href='index.php/archieve-list/add-archieve';</script>";
	}
	}
	...
	if ($_FILES["filearchieve"]["size"] > 500000) {
...
<table align="center">
	<tr>
		<td>Judul Dokumen</td>
		<td>:</td>
		<td><input type="text" name="nama_departement" class="form-control" required/></td>
	</tr>
	<tr>
		<td>Owner Dokumen</td>
		<td>:</td>
	...
	<tr>
		...
		<td>:</td>
		<td><input type="text" name="tanggal_revisi" id="tanggal"/></td>
	</tr>
	<tr>
	<td>Tambah file</td>
	<td>:</td>
	<td><input type="file" name="filearchieve" class='btn btn-primary'></td></tr>
	<tr>
	<tr>
