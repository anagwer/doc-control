...
// untuk memulai session pada fungsi login
	...
	if ($_GET['key']=='hapus'){
		...
		$qryDelete = "DELETE FROM sop WHERE id_sop='$id'";
		...
		echo "<script>window.location.href='index.php/sop/edit-form-sop';</script>";
		}
	...
	if(isset($_POST['submit'])){ //jika tombol submit di set atau dijalankan maka melakukan aksi
		...
		if ($_FILES['file_sop']['name']!=''){
		$target_dir = "uploads_file/all_doc/";
		$target_file = $target_dir . basename($_FILES["file_sop"]["name"]);
		$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
		if ($_FILES["file_sop"]["size"] > 1000000000) {
	...
	//}
		...
		if (move_uploaded_file($_FILES["file_sop"]["tmp_name"], $target_file)) {
				echo "File". basename( $_FILES["file_sop"]["name"]). " berhasil diupload.";

			} else {
				echo "Tidak ada file yang diunggah.";
			}
			
			$filename = basename($_FILES['file_sop']['name']);
			}
	...
	<?php
	$tampil = mysql_fetch_array(mysql_query("SELECT * FROM sop WHERE id_sop = '".$_GET['id_sop']."'")); 
	{
	?>
	<?php	
		 if(!empty($err))  {
	...
	<table align="center">
		...
		<!--tr>
			<td>Tambah file</td>
			<td>:</td>
			<td><input type="file" name="filename" value="<?=$tampil['file_sop']?>" class='btn btn-success'></td>
			<td><input type="text" name="filename" value="<?=$tampil['file_sop']?>" class="form-control" /></td>
		</tr-->
		<?php } ?>
		<tr>
	...
	echo "<tr>
		...
		if ($file_sop!=null){
		//echo "<td><a href='http://100.100.1.148/documentcontrol/uploads_file/doc_sop/".$file_sop."' title=".$file_sop.">".$code_document."</a></td>";
